...
<div class="mf-navigation-mobile" id="mf-navigation-mobile">
    <div class="navigation-list">
		...
		foreach ( $els as $el ) {
			if ('cart' == $el ) {			
				echo sprintf( '<a href="%s" class="navigation-icon navigation-mobile_home"><i class="icon-bag2"></i> %s</a>', esc_url( home_url( '/shop' ) ), esc_html( 'Shop' ) );			
			}	
		}
		foreach ( $els as $el ) {
			if ('search' == $el ) {		
				echo sprintf( '<a href="'. site_url('/my-account/').'" class="navigation-icon navigation-myaccount"><i class="icon-user"></i> %s</a>', esc_html( 'My Account' ) );
		...
		foreach ( $els as $el ) {
			if ( 'home' == $el || 'cat' == $el || 'search' == $el || 'cart' == $el  ) {
			/*if ( 'home' == $el ) {
				echo sprintf( '<a href="%s" class="navigation-icon navigation-mobile_home"><i class="icon-home"></i> %s</a>', esc_url( home_url( '/' ) ), esc_html( martfury_get_option( 'navigation_home_mobile' ) ) );
			} elseif ( 'cat' == $el ) {
				echo sprintf( '<a href="#" class="navigation-icon navigation-mobile_cat" id="navigation-mobile_cat"><i class="icon-menu"></i> %s</a>', esc_html( martfury_get_option( 'navigation_cat_mobile' ) ) );
