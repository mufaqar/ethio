...
class Martfury_WooCommerce {
	...
	function __construct() {
		...
		//add_action( 'woocommerce_no_products_found', array ( $this, 'action_woocommerce_no_products_found' ), 10 ); 
		add_action( 'woocommerce_no_products_found', array ( $this, 'shop_toolbar' ), 10 ); 
	...
	*/
	...
	function shop_toolbar() {
		...
		if ( in_array( 'filter', $elements ) ) {
			global $wp_query, $WCFM, $WCFMmp, $post,$WCFM_Query, $wpdb, $woocommerce, $wp;
			...
			/* Count All publish product start*/					
			$all_product_count = get_posts( $all_product );
			/* Count All publish product end*/
			
			/* Count country state and city start*/
			...
			$count_city = $count_state = $count_country = array();	
					...
					foreach( $wcfm_store_rep_location as $wcfm_store_rep_location_list ){
						foreach( $wcfm_store_rep_location_list as $wcfm_store_rep_location_list_array ){											
							foreach( $wcfm_store_rep_location_list_array as $custom_rep_location_list_array ){						
								//if(  $custom_rep_location_list_array['rep_country_value'] == $ip_country || $custom_rep_location_list_array['rep_state_value'] == $ip_state || $custom_rep_location_list_array['rep_city_value'] == $ip_city  ) {
								if(  $custom_rep_location_list_array['rep_country_value'] == $ip_country ) {
									$repeater_product_id_filter[] = $ps_detail->ID;	
									$count_country[] = $ps_detail->ID;								
								}										
							}
						}
					}
			...
			foreach( $custom_ps as $ps_detail ) {		
				...
				if( !empty( $wcfm_store_rep_location ) ){				
					foreach( $wcfm_store_rep_location as $wcfm_store_rep_location_list ){
						foreach( $wcfm_store_rep_location_list as $wcfm_store_rep_location_list_array ){											
							foreach( $wcfm_store_rep_location_list_array as $custom_rep_location_list_array ){						
								//if( $custom_rep_location_list_array['rep_state_value'] == $ip_state || $custom_rep_location_list_array['rep_country_value'] == $ip_country ) {
								if( $custom_rep_location_list_array['rep_state_value'] == $ip_state && $custom_rep_location_list_array['rep_country_value'] == $ip_country ) {
									$repeater_product_id_filter[] = $ps_detail->ID;	
									$count_state[] = $ps_detail->ID;
								}										
							}
						}
					}
			...
			foreach( $custom_ps as $ps_detail ) {		
				...
				if( !empty( $wcfm_store_rep_location ) ){					
					foreach( $wcfm_store_rep_location as $wcfm_store_rep_location_list ){						
						foreach( $wcfm_store_rep_location_list as $wcfm_store_rep_location_list_array ){							
							foreach( $wcfm_store_rep_location_list_array as $custom_rep_location_list_array ){									
								//if( $custom_rep_location_list_array['rep_city_value'] == $ip_city && $custom_rep_location_list_array['rep_city_value'] != '0' ) {
								if( $custom_rep_location_list_array['rep_city_value'] == $ip_city && $custom_rep_location_list_array['rep_country_value'] == $ip_country ) {
									$repeater_product_id_filter[] = $ps_detail->ID;	
									$count_city[] = $ps_detail->ID;									
								}												
							}
						}
					}
			...
			/*Get All country state and city count start*/
			echo '<ul class="ethio-country-count-hidden">';
			foreach( $custom_ps as $all_post_detail ) {
				$wcfm_store_rep_location = get_post_meta( $all_post_detail->ID, 'wcfm_store_rep_location_options', true );
				if( !empty( $wcfm_store_rep_location ) ){					
					foreach( $wcfm_store_rep_location as $wcfm_store_rep_location_list ){						
						foreach( $wcfm_store_rep_location_list as $wcfm_store_rep_location_list_array ){							
							foreach( $wcfm_store_rep_location_list_array as $custom_rep_location_list_array ){
								echo '<li  data-country="'. $custom_rep_location_list_array['rep_country_value'] .'" data-state="'. $custom_rep_location_list_array['rep_state_value'] .'" data-city="'. $custom_rep_location_list_array['rep_city_value'] .'" data-category-id="'. $current_term .'" data-product-id="'. $all_post_detail->ID .'">'.$custom_rep_location_list_array['rep_city_value'].'</li>';								
							}
						}
					}
				}
			...
			echo '</ul>';
			/*Get All country state and city count end*/
			
			/* Count country state and city end*/
			if( !empty ( $count_country )){ 
				$count_country_status = count( array_unique( $count_country ) );
			}else{ 
				$count_country_status = '0';				
			}
			if( !empty ( $count_state )){ 
				$count_state_status = count( array_unique( $count_state ) );
			}else{ 
				$count_state_status = '0';
			}
			if( !empty ( $count_city )){ 
				$count_city_status = count( array_unique(  $count_city ) );
			}else{ 
				$count_city_status =	'0';
			}				
			
			$output[] = '<div class="products-found all-product-filter"><a href="javascript:void(0)" class="ethio-filter-countryfilter">'. esc_html__( 'Country Filter', 'martfury' ) . '<i class="icon-equalizer"></i></a></div>';
			
			if( is_search() ) {
				$output[] = '<div class="products-found all-product"><a href="'. site_url('/?s='.$_GET['s'].'&post_type=product&lang='.$_GET['lang'].'&allcountry=yes' ) .'" class="ethio-filter-allcountry">'. esc_html__( 'All Country', 'martfury' ) . ' ('.  count( $all_product_count ) .')</a></div>';
			}else{			
				$output[] = '<div class="products-found all-product"><a href="javascript:void(0)" class="ethio-filter-allcountry">'. esc_html__( 'All Country', 'martfury' ) . ' ('.  count( $all_product_count ) .')</a></div>';
			}
			
			//$output[] = '<div class="products-found all-product-mobile"><a href="#" class="mf-filter-mobile" id="mf-filter-mobile"><i class="icon-equalizer"></i><span>Filter</span></a> ';
			/*if( $ip_country != 0 ){
				$output[] = '<div class="products-found all-product"><a href="javascript:void(0)" class="ethio-filter-country">'. esc_html__( '', 'martfury' ) . '</a><strong>(' . $count_country_status . ') </strong></div>';
			}
			
			if( $ip_state != 0 ){
				$output[] = '<div class="products-found all-product"><a href="javascript:void(0)" class="ethio-filter-state">'. esc_html__( '', 'martfury' ) . '</a><strong>(' . $count_state_status . ') </strong></div>';
			}
			if( $ip_city != 0 ){
				$output[] = '<div class="products-found all-product"><a href="javascript:void(0)" class="ethio-filter-city">'. esc_html__( '', 'martfury' ) . '</a><strong>(' . $count_city_status . ')  </strong></div>';
			}*/
			$output[] = '<input type="hidden" class="vendor_site_url_serach" value="'. site_url('shop') .'" />';
		}
