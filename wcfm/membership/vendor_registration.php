  ...
  <form id="wcfm_membership_registration_form" class="wcfm">
				 ...
				 if( $user_id ) {
					...
					if( $WCFM->is_marketplace && ($WCFM->is_marketplace == 'wcfmmarketplace') ) {
						...
						if( apply_filters( 'wcfm_is_allow_store_name', true ) && $WCFMmp->wcfmmp_vendor->is_vendor_sold_by() && !wcfm_is_vendor() ) {
							...
							echo '<p class="description wcfm_store_slug_verified wcfm_page_options_desc" data-avail="' . esc_html(__( 'Available', 'wc-multivendor-membership' )) . '" data-unavail="' . esc_html(__( 'Un-available', 'wc-multivendor-membership' )) . '">' . esc_url(trailingslashit( get_site_url() ) . $wcfm_store_url) . '/<span class="wcfm_store_slug">'.esc_attr($store_slug).'</span></p>';
						}
					}
					...
					if( !empty( $wcfmvm_registration_static_fields ) ) {
						foreach( $wcfmvm_registration_static_fields as $wcfmvm_registration_static_field => $wcfmvm_registration_static_field_val ) {
							...
							$wcfm_groups_array = get_posts( $args_group );
							
							$groups_array_stored = array();
							$groups_array_stored[0] = 'Select Business Type';
							foreach( $wcfm_groups_array as $wcfm_groups_list ){
								$id = $wcfm_groups_list->ID;
								$groups_array_stored[$id] = $wcfm_groups_list->post_title;
							}*/
							//array_unshift( $groups_array_stored, 'Select Business Type' );
							
							$wcfmvm_registration_static_fields = wcfm_get_option( 'wcfmvm_registration_static_fields', array() );
							$groups_array_stored = array();
							$groups_array_stored[0] = 'Select Business Type';
							if( !empty( $wcfmvm_registration_static_fields['group_list'] ) ){
								foreach( $wcfmvm_registration_static_fields['group_list'] as $group_id ){
									$groups_array_stored[$group_id] = get_the_title( $group_id );
								}
							}
							  ...
							  case 'address':
							  	...
							  	if( is_user_logged_in() && ( !isset( $field_value['country'] ) || ( isset( $field_value['country'] ) && empty( $field_value['country'] ) ) ) ) {
									...
									$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_membership_registration_fields_address', array(
										...
										"state" => array(
											'label' => __('State', 'wc-frontend-manager') , 
											'type' => 'select', 
										...
										"city" => array(
											'label' => __('City', 'wc-frontend-manager') , 
											'type' => 'select', 
											'name' => $field_name . '[city]', 
											'custom_attributes' => array( 'required' => 1 ), 
											'class' => 'wcfm-select wcfm_ele', 											
											'label_class' => 'wcfm_title wcfm_ele',
											'options'     =>array( '0' => 'Select City' ),
											'value' 	  => ''											
										...
										"addr_1" => array(
											...
											'type' => 'text', 'name' => $field_name . '[addr_1]', 
											'custom_attributes' => array( 'required' => 1 ), 
											'class' => 'wcfm-text wcfm_ele', 
											'label_class' => 'wcfm_title wcfm_ele', 
										...
										"addr_2" => array(
											...
											'type' => 'text', 'name' => $field_name . '[addr_2]', 
											'class' => 'wcfm-text wcfm_ele', 
											'label_class' => 'wcfm_title wcfm_ele', 
									...
									if( !apply_filters( 'wcfm_is_allow_sms_verification', true ) || !$sms_verification ) {
										...
										$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_membership_registration_fields_phone', array(
											"phone" => array(
												...
												'custom_attributes' => array( 'required' => 1 ), 
												'class' => 'wcfm-text wcfm_ele', 
												'label_class' => 'wcfm_title wcfm_ele', 'value' => $field_value 
											),
											"group_list"  => array( 
												'label' => __( 'Business Type', 'wc-frontend-manager' ), 
												'type' => 'select', 
												'name' => 'group_list',
												'custom_attributes' => array( 'required' => 1 ),
												'class' => 'wcfm-select wcfm_ele', //terms_page_ele
												'label_class' => 'wcfm_title', 
												'options' => $groups_array_stored, 
												'value' => ''
											)
										) ) );
					...
					// Terms of Use support add - 1.1.9
					if( $terms == 'active' ) {
						?>
						<input type="checkbox" id="terms" name="wcfmvm_static_infos[terms]" class="wcfm-checkbox" value="<?php esc_html_e( 'Agree', 'wc-multivendor-membership' ); ?>" data-required="1" data-required_message="<?php esc_html_e( 'Terms of Use', 'wc-multivendor-membership' ); ?>: <?php esc_html_e( 'This field is required.', 'wc-frontend-manager' ); ?>">
						<p class="terms_title wcfm_title">
							<strong>
								<span class="required">*</span>
								<?php 
								esc_html_e( 'I’ve read and agree with', 'wc-multivendor-membership' );
								echo '&nbsp;&nbsp;';
								if( $terms_page ) {
									?><a target="_blank" href="<?php echo esc_url(get_permalink( $terms_page )); ?>"><?php esc_html_e( 'Privacy policy', 'wc-multivendor-membership' ); ?></a><span><?php esc_html_e( ' & ', 'wc-multivendor-membership' );  ?></span> <a target="_blank" href="<?php echo esc_url(site_url( 'terms-of-use' )); ?>"><?php esc_html_e( 'Term of Use', 'wc-multivendor-membership' ); ?></a><?php
								} else {
									esc_html_e( 'Terms of Use', 'wc-multivendor-membership' );
								}
								?>
							</strong>
						</p>
		...
		<div id="wcfm_membership_registration_submit" class="wcfm_form_simple_submit_wrapper">
		  <?php if( wcfm_is_allowed_membership() ) { ?>
			  <input type="submit" name="save-data" value="<?php if( $user_id ) { esc_html_e( 'Confirm', 'wc-multivendor-membership' ); } else { esc_html_e( 'Register', 'wc-multivendor-membership' ); } ?>" id="wcfm_membership_register_button" class="wcfm_submit_button" />
			  <?php if( !apply_filters( 'wcfmvm_is_allow_registration_first', false ) && is_wcfm_membership_page() ) { ?>
			  <a href="<?php echo esc_url(apply_filters( 'wcfm_change_membership_url', get_wcfm_membership_url() )); ?>" class="wcfm_submit_button"><<&nbsp;<?php esc_html_e( 'Plans', 'wc-multivendor-membership' ); ?></a>
			  <?php } ?>
			<?php } else { ?>
				<?php esc_html_e( 'Your user role not allowed to subscription!', 'wc-multivendor-membership' ); ?>
			<?php } ?>
		</div>
		<div class="wcfm-clearfix"></div>
	</form>
